...
function coopkong.startplugin()
	...
	function initialize()
		...
		if mac ~= nil then
			...
			if session == 2 then
				for k, v in ipairs({0x0cd8, 0x0ceb, 0x0cf6, 0x0cbf, 0x0679}) do mem:write_direct_u8(v, 0x00) end
			end
	...
	function write_data(address, values)
		for k, v in ipairs(values) do
			mem:write_direct_u8(address + k - 1, v)
		end
	end
	...
	function read_score(start_address, length)
		-- read score from video ram
		local text = ""
		for i=start_address, start_address - (0x20 * length - 1), -0x20 do
			text = text..tostring(mem:read_u8(i))
		end
		return text
	end
	
	function write_message(start_address, text, adjust)
		...
		for key=1, string.len(text) do
			...
			if _p and _p > 0 then
				mem:write_u8(start_address - ((key - 1) * 32), _p - 1 + _adjust)
			else
				mem:write_u8(start_address - ((key - 1) * 32), 0xb0)
			end
		end
	end	
	...
	function write_rom_message(start_address, text)
		...
		for key=1, string.len(text) do
			mem:write_direct_u8(start_address + (key - 1), string.find(characters, string.sub(text, key, key)) - 1)
		end
	end	
